...
function* handleFetchTodos() {
  ...
  if (responseJson)
    yield put({ type: TODOS_INDEX_SUCCESS, responseJson });
...
function* handlePostTodo(payload) {
  ...
  if (responseJson)
    yield put({ type: TODO_POST_SUCCESS, responseJson });
  else yield put({ type: TODOS_ERROR, error });
}

function* handleEditTodo(payload) {
  const { responseJson, error } = yield call(editTodo, payload);
  if (responseJson)
    yield put({ type: TODO_EDIT_SUCCESS, responseJson });
  else yield put({ type: TODOS_ERROR, error });
}

function* handlePostReminderTodo(payload) {
  const { responseJson, error } = yield call(
    postReminderTodo,
    payload,
  );
  if (responseJson)
...
function* handleDeleteTodos(payload) {
  ...
  if (responseJson)
    yield put({ type: TODOS_DELETE_SUCCESS, responseJson });
  else yield put({ type: TODOS_ERROR, error });
}
